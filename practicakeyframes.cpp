...
int main()
{
	...
	camera = Camera(glm::vec3(0.0f, 8.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), -60.0f, 0.0f, 5.0f, 0.5f);
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Escaleras
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(10.0f, 0.25f, 0.0f));
		...
		//base
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(11.75f, 0.5f, -1.75f));
		...
		Tarbol1.UseTexture(); // el agave bonito con transparencia
		Material_opaco.UseMaterial(uniformSpecularIntensity, uniformShininess);
		...
		glEnable(GL_BLEND);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		Tarbol2.UseTexture(); // el agave bonito con transparencia
		Material_opaco.UseMaterial(uniformSpecularIntensity, uniformShininess);
		meshList[3]->RenderMesh();
		glDisable(GL_BLEND);
		...
		glEnable(GL_BLEND);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		Tarbol3.UseTexture(); // el agave bonito con transparencia
		Material_opaco.UseMaterial(uniformSpecularIntensity, uniformShininess);
		meshList[3]->RenderMesh();
		glDisable(GL_BLEND);
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(movCoche, -1.4f, 0.5f)); //Tralasion
		model = glm::scale(model, glm::vec3(0.1f, 0.1f, 0.1f));//Escala
		model = glm::rotate(model, 90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f)); //Rotacion
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Material_opaco.UseMaterial(uniformSpecularIntensity, uniformShininess);
		Kitt_M.RenderModel(); //carga del carro
		Camino_M.RenderModel();//carga del camino (rieles)
...
int main() 
{
	...
	camera = Camera(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), -60.0f, 2.0f, 5.0f, 0.5f);
